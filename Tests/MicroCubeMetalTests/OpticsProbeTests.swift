...
final class OpticsProbeTests: XCTestCase {
    ...
    private func runOpticsProbe() throws -> Report {
        let source = """
        kernel void probeOptics(device float *output [[buffer(0)]], uint gid [[thread_position_in_grid]]) {
            if (gid != 0u) return;
            ...
            shadeSecondaryLighting(
                float3(0.0f), float3(0.0f, 1.0f, 0.0f), float3(1.0f),
                float3(0.0f, 1.0f, 0.0f), 0.42f, secondaryOpticalRays
            );
            ...
            output[10] = float(secondaryOpticalRays);
        }
        """
        ...
        let output = try XCTUnwrap(device.makeBuffer(length: 11 * MemoryLayout<Float>.stride, options: .storageModeShared))
        ...
        let values = output.contents().bindMemory(to: Float.self, capacity: 11)
        ...
        return Report(
            ...
            recursiveSecondaryRayCount: Int(values[10])
        )
    }
