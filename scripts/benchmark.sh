...
lipo -archs "$EXECUTABLE" 2>/dev/null | tr ' ' '\n' | grep -qx 'arm64' || { print -u2 -- "Benchmark requires an arm64 executable."; exit 1; }
...
validate_report() {
    ...
    jq -e --argjson pixels "[${size%x*}, ${size#*x}]" '
        .schemaVersion == 1 and .status == "pass" and .failure == null and .device == "Apple M4 Max" and
        (.os | type == "string" and length > 0) and .scene == "hero" and .fixedTime == 0 and
        .thermalStateBefore == "nominal" and .thermalStateAfter == "nominal" and
        .drawablePixels == $pixels and .renderScale == 1 and .windowCount == 1 and
        .productionKernels == ["generateTerrain", "reduceOccupancy", "buildMixedOccupancy", "reduceMixedOccupancy", "clearVolumeLighting", "injectVolumeLighting", "raycastHybrid"] and .fixedStep == (1 / 120) and
...
for size in 1280x800 2560x1600; do
    ...
    jq -e -s --argjson threshold "$threshold" 'map(.p95GPUms) | max <= $threshold' "${reports[@]}" >/dev/null || {
